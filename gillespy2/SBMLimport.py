...
def convert(filename, model_name=None, gillespy_model=None):
    try:
        ...
        raise ImportError('libsbml is required to convert SBML files for GillesPy2.')
    ...
    if gillespy_model is None:
        gillespy_model = Model(name=model_name)
    ...
    for i in range(model.getNumParameters()):
        ...
        gillespy_parameter = Parameter(name=name, expression=value)
    ...
    for i in range(model.getNumReactions()):
        ...
        gillespy_reaction = Reaction(name=name, reactants=reactants, products=products, propensity_function=propensity)
