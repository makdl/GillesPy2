    ...
    def run(self, model, t=20, number_of_trajectories=1,
        ...
        if stochkit_home is not None:
            if os.path.isfile(os.path.join(stochkit_home, algorithm)):
                ...
                raise SimulationError("stochkit executable '{0}' not found stochkit_home={1}".format(algorithm,
                                                                                                     stochkit_home))
            ...
            if os.environ.get('PATH') is not None:
                for dir in os.environ.get('PATH').split(':'):
                    if os.path.isfile(os.path.join(dir, algorithm)):
                        ...
                        break
        if executable is None:
            raise SimulationError("stochkit executable '{0}' not found. Make sure it is your path, or set STOCHKIT_\
            HOME envronment variable'".format(algorithm))
        ...
        try:
            ...
            fname = os.path.join(prefix_basedir, 'temp_input_{0}_generated_code'.format(ensemblename),
                                 'compile-log.txt')
...
class StochKitODESolver(GillesPySolver):
    ...
    @classmethod
    def run(cls, model, t=20, number_of_trajectories=1, increment=0.05, seed=None, stochkit_home=None,
            algorithm='stochkit_ode.py', job_id=None, debug=False, show_labels=False):
